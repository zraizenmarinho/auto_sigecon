...
def obter_hora_AP_TAG(file_path):
    class HorasPorTipoFinanciamento:
        def __init__(self, file_path):
            self.data = pd.read_excel(file_path)
    ...
    for mes_atual, mes_referencia in meses.items():
        for tipo_financiamento in ['1 Gratuidade Regimental', '2 Gratuidade Não Regimental', '3 Convênio', '9 Pago por Pessoa Fisica ou Empresa']:
            chave_resultado = f"{mes_atual}_ap_ha_{tipo_financiamento}"
            resultados_ha[chave_resultado] = hora_aluno_por_tipo.somar_hora('1117376 SENAI Taguatinga', '11 Aprendizagem Industrial básica', '1 Presencial', mes_referencia, tipo_financiamento)
    ...
    for idx, (mes_atual, mes_referencia) in enumerate(meses.items()):
        ...
        for tipo_financiamento in ['1 Gratuidade Regimental', '2 Gratuidade Não Regimental', '3 Convênio', '9 Pago por Pessoa Fisica ou Empresa']:
            chave_resultado_atual = f"{mes_atual}_ap_ha_{tipo_financiamento}"
            chave_resultado_anterior = f"{list(meses.keys())[idx-1]}_ap_ha_{tipo_financiamento}"
            
            resultado_atual = resultados_ha[chave_resultado_atual]
            soma_meses_anteriores = sum(resultados_ha[f"{list(meses.keys())[i]}_ap_ha_{tipo_financiamento}"] for i in range(idx))
            resultados_ha[chave_resultado_atual] -= soma_meses_anteriores
...
def obter_hora_QP_TAG(file_path):
    class HorasPorTipoFinanciamento:
        def __init__(self, file_path):
            self.data = pd.read_excel(file_path)
    ...
    for mes_atual, mes_referencia in meses.items():
        for tipo_financiamento in ['1 Gratuidade Regimental', '2 Gratuidade Não Regimental', '3 Convênio',
            ...
            chave_resultado = f"{mes_atual}_qp_ha_{tipo_financiamento}"
            resultados_ha[chave_resultado] = hora_aluno_por_tipo.somar_hora('1117376 SENAI Taguatinga',
                                                                            '21 Qualificação Profissional',
                                                                            '1 Presencial', mes_referencia,
                                                                            tipo_financiamento)
    ...
    for idx, (mes_atual, mes_referencia) in enumerate(meses.items()):
        ...
        for tipo_financiamento in ['1 Gratuidade Regimental', '2 Gratuidade Não Regimental', '3 Convênio',
                                   '9 Pago por Pessoa Fisica ou Empresa']:
            chave_resultado_atual = f"{mes_atual}_qp_ha_{tipo_financiamento}"
            chave_resultado_anterior = f"{list(meses.keys())[idx - 1]}_qp_ha_{tipo_financiamento}"

            resultado_atual = resultados_ha[chave_resultado_atual]
            soma_meses_anteriores = sum(
                resultados_ha[f"{list(meses.keys())[i]}_qp_ha_{tipo_financiamento}"] for i in range(idx))
            resultados_ha[chave_resultado_atual] -= soma_meses_anteriores
